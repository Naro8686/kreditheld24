...
class ProposalController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            if (request()->ajax()) {
                return $this->ajaxDataTable(auth()->user()->proposals()
            ...
            $totalSum
                = Proposal::moneyFormat($successful->sum('proposals.creditAmount'));

            $targetPercent = $user->targetPercent($startOfYear->toDateString());

            $monthSum
                = Proposal::moneyFormat($successful->where('proposals.created_at',
                '>=', $startOfYear > now()->subMonth() ? $startOfYear
                    : now()->subMonth())->sum('proposals.creditAmount'));
            return view('proposal.index',
                compact('totalSum', 'monthSum', 'targetPercent'));
        } catch (Throwable $e) {
        }
    ...
    {
        if (request()->ajax()) {
            return $this->ajaxDataTable(auth()
                ->user()->proposals()
    ...
    {
        $user = auth()->user();
        $proposal = Proposal::whereNull('proposals.archived_at')
            ->withTrashed()
            ->when(!$user->isAdmin(), function($query) use ($user) {
                return $query->where('proposals.user_id', $user->id);
            })
            ->findOrFail($id);
    ...
    public function defaultFields(
        ...
        $success = $proposal->saveData($request->only($default),
            $request->get('allFilesName', []));
        $redirectUrl = $isDraft ? route('proposal.draft')
            : route('proposal.index');
        return response()->json([
            'message'     => $success
                ? $isDraft ? __("Saved.") : __("Application sent")
                : __("Whoops! Something went wrong."),
            'redirectUrl' => $redirectUrl, 'success' => $success
        ], $success ? 200 : 500);
    }
    ...
    private function ajaxDataTable($proposalsBuilder)
    {
        return DataTables::eloquent($proposalsBuilder)
            ...
            ->addColumn('fullName', function(Proposal $proposal) {
                ...
                $html
                    = "<div class='row-actions d-flex align-items-center' role='group'>";
            ...
            ->addColumn('action', function(Proposal $proposal) {
                ...
                if ($proposal->trashed() || $proposal->isRevision()
                    || $proposal->isApproved()
                ) {
    ...
    public function duplicate($id)
    {
        ...
        $newProposal->save();
        $currentRouteName = is_null($newProposal->deleted_at)
            ? $archiveRouteName : $draftRouteName;
